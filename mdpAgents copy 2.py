...
class Grid:
    ...
    def __init__(self, width, height, walls, food, capsules, ghostsWithDirections):
        self.width = width
        self.height = height
        self.utility_grid = [[0 for _ in range(width)] for _ in range(height)]
        self.reward_grid = [[0 for _ in range(width)] for _ in range(height)] 
        self.ghostReward = -130
        self.foodReward = 70
        self.finalFoodReward = self.foodReward
        self.capsuleReward = 30
        self.walls = walls
        self.furthestDistance = 0
        self.rateIfNotCloser = 0.03
        self.ghostAura = 4
        ...
        for x in range(width):
            for y in range(height):
                if (x, y) in food:
                    if len(food) == 1:
                        self.updateReward((x, y), self.getReward((x, y)) + self.finalFoodReward)
                    else:
                        self.updateReward((x, y), self.getReward((x, y)) + self.foodReward)   
                elif (x, y) in capsules:
                    self.updateReward((x, y), self.getReward((x, y)) + self.capsuleReward)
                elif (x, y) in walls:
        ...
        self.checkGhosts = False

        for ghost in ghostsWithDirections:
            if ghost['dir'] != Directions.STOP:
                self.checkGhosts = True
                break

        if self.checkGhosts:
            if ghostsWithDirections:
                self.getFurthestDistance(ghostsWithDirections[0])
    ...
    def updateSquareReward(self, coord):
        ...
        if numOfWalls == 3:
            self.updateReward((x, y), self.getReward((x, y)) + -60)
        elif numOfWalls == 2:
            self.updateReward((x, y), self.getReward((x, y)) + -10)
        elif numOfWalls == 1:
            self.updateReward((x, y), self.getReward((x, y)) + -5)
    ...
    def updateNeighboursRewards(self, ghost):
        ...
        next_pos = self.get_next_position((x,y), oppositeDirections[ghost['dir']])
        if next_pos != (x, y) and next_pos not in self.walls:
            self.updateReward((x, y), self.getReward((x, y)) + ((1 - (float(1) / (self.ghostAura + 1))) * self.ghostReward))
...
class MDPAgent(Agent):
    def __init__(self):
        ...
        self.ghost_positions_last_step = []
